...
def stop_attacks(message):
    ...
    try:
        if user_id != 1695959688:
            ...
            if not user_data or user_data['plan'] < 2:
                bot.send_message(chat_id, "*Permission denied*", parse_mode='Markdown')
                return
        ...
        bot.send_message(chat_id, f"*Stopped {killed_count} active attacks*", parse_mode='Markdown')
        
    except Exception as e:
        logging.error(f"Stop command error: {e}")
        bot.send_message(chat_id, "*Error stopping attacks*", parse_mode='Markdown')
